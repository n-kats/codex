...
| `custom.exec.worker_user`                        | string                                                            | (Forks) Run command-executing tool calls as this OS user (on Unix, `exec_command` uses `sudo -n` to switch users).              |
| `custom.exec.worker_uid`                         | number                                                            | (Forks) Run command-executing tool calls as this UID (requires `custom.exec.worker_gid`; on Unix, `exec_command` uses `sudo -n`). |
| `custom.exec.worker_gid`                         | number                                                            | (Forks) Run command-executing tool calls as this GID (requires `custom.exec.worker_uid`; on Unix, `exec_command` uses `sudo -n`). |
