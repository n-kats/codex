...
.PHONY: \
	help fetch cache-dir download-assets \
	release-dir release \
	docker-build \
	fmt \
	build \
...
CARGO_MCP_TARGET_DIR := $(ROOT_DIR)/_tmp/codex_build/codex_cache
CODEX_DOCKER_TARGET_DIR ?= $(CARGO_MCP_TARGET_DIR)
...
CARGO_TEST_FLAGS ?=
# Cargo's build timing report is enabled for test targets by default. Set this
# to empty to disable it.
CARGO_TIMINGS_FLAGS ?= --timings

# Quote a complete shell command as one argument without creating a temporary file.
shell_quote = '$(subst ','\'',$(1))'

# Use Cargo's standard profiles for non-release Rust targets.
V8_FROM_SOURCE ?= 0
V8_ASSETS_DIR ?= $(ROOT_DIR)/_tmp/assets
V8_ASSETS_DIR_DOCKER ?= /workspace/_tmp/assets
V8_TARGET ?= $(shell rustc -vV 2>/dev/null | awk '/^host: / { print $$2; exit }')
V8_VERSION := $(shell awk '/^name = "v8"$$/ { found=1 } found && /^version = / { gsub(/"/, "", $$3); print $$3; exit }' "$(CODEX_RS_DIR)/Cargo.lock")
V8_ARTIFACT_PROFILE := ptrcomp_sandbox_release
V8_ARCHIVE_NAME := $(if $(findstring pc-windows-msvc,$(V8_TARGET)),rusty_v8_$(V8_ARTIFACT_PROFILE)_$(V8_TARGET).lib.gz,librusty_v8_$(V8_ARTIFACT_PROFILE)_$(V8_TARGET).a.gz)
V8_BINDING_NAME := src_binding_$(V8_ARTIFACT_PROFILE)_$(V8_TARGET).rs
V8_ARCHIVE_PATH := $(V8_ASSETS_DIR)/$(V8_ARCHIVE_NAME)
V8_BINDING_PATH := $(V8_ASSETS_DIR)/$(V8_BINDING_NAME)
V8_ARCHIVE_PATH_DOCKER := $(V8_ASSETS_DIR_DOCKER)/$(V8_ARCHIVE_NAME)
V8_BINDING_PATH_DOCKER := $(V8_ASSETS_DIR_DOCKER)/$(V8_BINDING_NAME)
ifneq ($(and $(wildcard $(V8_ARCHIVE_PATH)),$(wildcard $(V8_BINDING_PATH))),)
V8_ASSET_EXPORTS := export RUSTY_V8_ARCHIVE=$(V8_ARCHIVE_PATH_DOCKER); export RUSTY_V8_SRC_BINDING_PATH=$(V8_BINDING_PATH_DOCKER);
else
V8_ASSET_EXPORTS :=
endif
...
CARGO_NON_RELEASE_EXPORTS := export CARGO_BUILD_JOBS=$(CARGO_BUILD_JOBS); export V8_FROM_SOURCE=$(V8_FROM_SOURCE); $(V8_ASSET_EXPORTS)

# Release builds use the same downloaded Codex V8 artifacts as normal builds.
CARGO_RELEASE_EXPORTS := export V8_FROM_SOURCE=$(V8_FROM_SOURCE); $(V8_ASSET_EXPORTS)

CARGO_TEST_NON_RELEASE_EXPORTS := export CARGO_BUILD_JOBS=$(CARGO_BUILD_JOBS); export V8_FROM_SOURCE=$(V8_FROM_SOURCE); $(V8_ASSET_EXPORTS)

download-assets:
	@V8_ASSETS_DIR="$(V8_ASSETS_DIR)" V8_TARGET="$(V8_TARGET)" V8_VERSION="$(V8_VERSION)" bash "$(ROOT_DIR)/scripts/downloads_assets.sh"
...
clean: docker-build
	@# The MCP target is a symlink to an external filesystem and is not mounted into this Docker runner.
	@if [ -L "$(CARGO_MCP_TARGET_DIR)" ] || [ -d "$(CARGO_MCP_TARGET_DIR)" ]; then shopt -s dotglob nullglob; rm -rf -- "$(CARGO_MCP_TARGET_DIR)"/*; shopt -u dotglob nullglob; fi
...
clean-dry-run: docker-build
	@echo "MCP_CARGO_TARGET_DIR=$(CARGO_MCP_TARGET_DIR)"; if [ -L "$(CARGO_MCP_TARGET_DIR)" ] || [ -d "$(CARGO_MCP_TARGET_DIR)" ]; then echo "[would remove] $(CARGO_MCP_TARGET_DIR)/*"; find -L "$(CARGO_MCP_TARGET_DIR)" -mindepth 1 -maxdepth 1 -print | sort; else echo "[missing] $(CARGO_MCP_TARGET_DIR)"; fi
...
define run_test_logged
	...
	CODEX_DOCKER_TARGET_DIR="$(CODEX_DOCKER_TARGET_DIR)" \
	bash "$(ROOT_DIR)/scripts/run_logged.sh" "$(1)" $(call shell_quote,$(2))
endef
...
help:
	@printf "%s\n" "Common commands:" \
		...
		"  (Docker target dir: $(CODEX_DOCKER_TARGET_DIR))" \
		...
		"  (Non-release Rust builds use Cargo's standard profiles)" \
		...
		"  make download-assets  # Download cached V8 artifacts into ./_tmp/assets" \
		...
		"  make test-normal      # Alias for test-almost" \
		"  make test-almost      # Run one cargo test with normal + flaky skips" \
		"  make test-flaky       # Alias for test-almost" \
		"  make update-fixtures  # Regenerate config schema + accept snapshots" \
		"  make all              # Run format + test-all" \
		"  make almost           # Run format + one skipped-test cargo run" \
...
fmt: cache-dir docker-build
	$(call run_test_logged,fmt,cd codex-rs && cargo +nightly fmt)

write-config-schema: cache-dir docker-build
	$(call run_test_logged,write_config_schema,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo run -p codex-core --bin codex-write-config-schema)
...
build: cache-dir docker-build
	$(call run_test_logged,build_cli,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo build -p codex-cli --bin codex)

release: cache-dir release-dir docker-build
	$(call run_test_logged,release,$(CARGO_RELEASE_EXPORTS) /bin/bash "$(ROOT_DIR_DOCKER)/scripts/release_linux.sh" "$(ROOT_DIR_DOCKER)" "$(CODEX_RS_DIR_DOCKER)" "$(RELEASE_DIR_DOCKER)")
...
test-core: build-linux-sandbox docker-build
	$(call run_test_logged,test_core,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test $(CARGO_TEST_FLAGS) -p codex-core)

test-all: build-linux-sandbox docker-build
	$(call run_test_logged,test_all,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test $(CARGO_TEST_FLAGS) --all-features)

all:
	$(call run_targets_continue_logged,all,fmt test-all)

test-almost: build-linux-sandbox docker-build
	@# Keep the normal and flaky skip lists in one cargo test invocation, as in the v20260703 Makefile.
	$(call run_test_logged,test_almost,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && printf '%s\n' "build diagnostics: V8_FROM_SOURCE=$(V8_FROM_SOURCE)" "build diagnostics: CARGO_TIMINGS_FLAGS=$(CARGO_TIMINGS_FLAGS)" "test diagnostics: phase=start" && TIMEFORMAT='test diagnostics: phase=finished elapsed_seconds=%R'; time cargo test $(CARGO_TEST_FLAGS) $(CARGO_TIMINGS_FLAGS) -- $(foreach test,$(SKIP_ALMOST_TESTS),--skip $(test)))

test-normal: test-almost

test-flaky: test-almost
...
verify-additional-prompt-dirs-env: cache-dir docker-build
	$(call run_test_logged,verify_additional_prompt_dirs_parse,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib custom__追加プロンプトディレクトリ__カンマ区切りと相対パスを解決できる)
	$(call run_test_logged,verify_additional_prompt_dirs_discover,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib custom__追加プロンプトディレクトリ__同名は後勝ちで名前順に並ぶ)

verify-exec-command-default-login: build-linux-sandbox docker-build
	$(call run_test_logged,verify_exec_command_default_login,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-app-server --test all suite::v2::turn_start::command_execution_notifications_include_process_id)
	$(call run_test_logged,verify_exec_command_default_login_derive_exec_args,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib shell_tests::derive_exec_args)
	$(call run_test_logged,verify_exec_command_default_login_shell_startup_files,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib custom__シェル起動ファイル__)
	$(call run_test_logged,verify_exec_command_default_login_cli_flag,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-cli --bin codex custom__shell_startup_files_cli_flag__)

verify-linux-default-shell: cache-dir docker-build
	$(call run_test_logged,verify_linux_shell_detect_zsh,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib shell::tests::test_current_shell_detects_zsh)
	$(call run_test_logged,verify_linux_bash_snapshot_sections,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib shell_snapshot::tests::linux_bash_snapshot_includes_sections)
	$(call run_test_logged,verify_linux_sh_snapshot_sections,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib shell_snapshot::tests::linux_sh_snapshot_includes_sections)
	$(call run_test_logged,verify_linux_snapshot_file_lifecycle,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-core --lib shell_snapshot::tests::try_new_creates_and_deletes_snapshot_file)
...
test-tui: cache-dir docker-build
	$(call run_test_logged,test_tui,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test -p codex-tui)

test-custom: build-linux-sandbox docker-build
	$(call run_test_logged,test_custom,$(CARGO_NON_RELEASE_EXPORTS) cd codex-rs && cargo test custom__)
