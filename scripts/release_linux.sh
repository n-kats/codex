...
code_mode_host_bin_path="${target_dir}/release/codex-code-mode-host"
if [[ ! -x "$code_mode_host_bin_path" ]]; then
  echo "ERROR: expected built binary at ${code_mode_host_bin_path}" >&2
  exit 1
fi

cp "$bin_path" "$stage_dir/codex"
cp "$code_mode_host_bin_path" "$stage_dir/codex-code-mode-host"

tar_entries=("codex" "codex-code-mode-host")
...
if command -v strip >/dev/null 2>&1; then
  ...
  strip "$stage_dir/codex" "$stage_dir/codex-code-mode-host"
  strip_used="yes"
else
...
- この tarball には \`codex-code-mode-host\` を同梱しています。
