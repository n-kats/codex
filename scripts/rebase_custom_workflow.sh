...
if git -C "${repo_dir}" show-ref --verify --quiet "refs/heads/${date_branch}"; then
  echo "${date_branch} already exists; stop to avoid overwriting the pre-rebase branch" >&2
  exit 1
fi
...
echo "creating ${date_branch} from ${work_branch}..."
git -C "${repo_dir}" branch "${date_branch}"
...
echo "creating ${backup_branch} from squashed ${work_branch}..."
...
echo "date_branch=${date_branch} ($(git -C "${repo_dir}" rev-parse --short "${date_branch}"))"
