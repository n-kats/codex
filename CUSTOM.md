...
- `Makefile` は、利用者から明示的に変更を依頼された場合に限り編集する。ビルド診断・高速化・ログ追加・依存削減などを理由に、エージェントが独断で既存のビルド処理、環境変数、Cargo flags、テスト対象を変更してはならない。
...
- （機能除去）Cloud Tasks の除去: ローカル CUI 利用では Cloud Tasks を使わないため、`codex cloud` サブコマンドと関連 crate 群を custom 側から除去し、`codex-cli` の通常ビルド・workspace テストに `codex-core` 経由の Cloud Tasks / V8 依存を持ち込まない。上流 rebase 時は Cloud Tasks の追加・変更を custom 差分として再確認する（詳細: `_docs/custom_notes/remove_cloud_tasks_command/README.md`）。
