...
- `_docs/custom_notes/rebase_hints/README.md`: 上流更新取り込み（rebase）のヒント、`range-diff` による差分検査、よく直したパターン集
...
- `_docs/custom_notes/rebase_rules/README.md`: custom ブランチの rebase 運用ルール（squash、tmp-rebase、range-diff）
- `_docs/custom_notes/docker_test_env/README.md`: Docker ベースのビルド/テスト環境（Makefile の Docker 化）
- `_docs/custom_notes/tool_parallelism_test/README.md`: tool parallelism テストの安定化（時間依存を排除）
- `_docs/custom_notes/exec_server_tests_dotslash/README.md`: exec-server テストで dotslash を必須化（Docker イメージに同梱）
