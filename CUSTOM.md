...
- （機能追加）コマンド実行の権限分離: モデルが実行する `shell` / `shell_command` / `exec_command` を worker ユーザー（例: `assistant`）に固定できる（設定は `custom.exec.*`。安全のため `shell_environment_policy.inherit = "all"` との併用はエラー。`!` は invoker のまま。詳細: `_docs/custom_notes/command_exec_worker_user/README.md`）。
