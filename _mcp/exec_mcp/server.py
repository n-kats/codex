...
def _configure_v8_assets(root_dir: Path, env: dict[str, str]) -> None:
    root_resolved = root_dir.resolve()
    explicit_paths: dict[str, Path] = {}
    for env_name in ("RUSTY_V8_ARCHIVE", "RUSTY_V8_SRC_BINDING_PATH"):
        ...
        if resolved.is_relative_to(root_resolved) and resolved.is_file():
            explicit_paths[env_name] = resolved
        else:
            logging.getLogger("exec_mcp").warning(
                "%s は MCP root 配下の既存ファイルだけ許可: root=%s path=%s",
                env_name,
                root_resolved,
                resolved,
            )
    ...
    if len(explicit_paths) == 2:
        env.update({name: str(path) for name, path in explicit_paths.items()})
        return

    env.pop("RUSTY_V8_ARCHIVE", None)
    env.pop("RUSTY_V8_SRC_BINDING_PATH", None)
    assets_dir = root_resolved / "_tmp" / "assets"
    binding_prefix = "src_binding_ptrcomp_sandbox_release_"
    for binding_path in sorted(assets_dir.glob(f"{binding_prefix}*.rs")):
        target = binding_path.name.removeprefix(binding_prefix).removesuffix(".rs")
        archive_candidates = (
            assets_dir / f"librusty_v8_ptrcomp_sandbox_release_{target}.a.gz",
            assets_dir / f"rusty_v8_ptrcomp_sandbox_release_{target}.lib.gz",
        )
        archive_path = next(
            (candidate for candidate in archive_candidates if candidate.is_file()), None
        )
        if archive_path is None:
            continue
        env["RUSTY_V8_ARCHIVE"] = str(archive_path)
        env["RUSTY_V8_SRC_BINDING_PATH"] = str(binding_path)
        return
...
def _codex_env_report(root_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "raw_env": {
            ...
            "CODEX_DOCKER_CACHE_DIR": raw_env.get("CODEX_DOCKER_CACHE_DIR"),
        ...
        "mounts": {
            "cargo": f"{cache_dir}/cargo -> {cargo_home}",
            "rustup": f"{cache_dir}/rustup -> {rustup_home}",
            "home": f"{cache_dir}/home -> {home_dir}",
