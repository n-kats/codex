...
struct MultitoolCli {
    ...
    ///
    /// When provided, Codex will read these files in order and will error if
    /// any path is missing.
    #[arg(
        long = "agents-md",
        global = true,
...
async fn cli_main(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    match subcommand {
        None => {
            ...
            let exit_info =
                run_interactive_tui(interactive, codex_linux_sandbox_exe, agents_md.clone())
                    .await?;
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            codex_exec::run_main(exec_cli, codex_linux_sandbox_exe, agents_md.clone()).await?;
        }
...
async fn run_interactive_tui(
    ...
    codex_tui::run_main(interactive, codex_linux_sandbox_exe, agents_md).await
}
