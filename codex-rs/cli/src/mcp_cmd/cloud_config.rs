...
pub(super) async fn load_mcp_config(
    ...
    let _bootstrap_config = load_config_toml_with_layer_stack(
        codex_home.as_path(),
        Some(&cwd),
        cli_overrides.clone(),
        ConfigLoadOptions {
    ...
    // Cloud-managed configuration is retained upstream but disabled for the
    // custom CLI. Keep the upstream fetch path documented here without
    // importing codex-cloud-config into the normal CLI build.
