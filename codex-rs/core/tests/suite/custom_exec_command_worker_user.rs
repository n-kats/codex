...
#[cfg(unix)]
fn parse_exec_output_text(output_text: &str) -> (Option<i32>, String) {
...
async fn custom__exec_worker_user__exec_command_tty_false_runs_as_worker_user() -> Result<()> {
    ...
    let request_log = mount_sse_sequence_unchecked(
        harness.server(),
        vec![
            sse(vec![
...
#[cfg(unix)]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn custom__exec_worker_user__apply_patch_respects_worker_user_permissions() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));
    skip_if_windows!(Ok(()));
    ...
    let call_id = "apply-patch-worker-user";
...
*** End Patch"#;
    ...
    let harness = TestCodexHarness::with_builder(builder).await?;
    harness.write_file("locked.txt", "original\n").await?;
    let locked_path = harness.path("locked.txt");
    ...
    codex
        .submit(Op::UserTurn {
            environments: None,
            items: vec![UserInput::Text {
                text: "apply the locked file patch".into(),
                text_elements: Vec::new(),
            }],
            final_output_json_schema: None,
            cwd,
    ...
    let end = wait_for_event_match(&codex, |event| match event {
        EventMsg::PatchApplyEnd(end) if end.call_id == call_id => Some(end.clone()),
        _ => None,
    })
    .await;
    ...
    assert!(
        !end.success,
        "expected apply_patch to fail under worker_user"
    );
    let stderr = end.stderr.to_lowercase();
    assert!(
    ...
    assert_eq!(harness.read_file_text("locked.txt").await?, "original\n");
    ...
    let output_text = request_log
        ...
        .unwrap_or_default();
    let (exit_code, output_body) = parse_exec_output_text(&output_text);
    assert_eq!(exit_code, Some(1), "output={output_text}");
    assert!(
        output_body.to_lowercase().contains("permission denied")
            || output_body
                .to_lowercase()
