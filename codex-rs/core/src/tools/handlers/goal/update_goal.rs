...
impl ToolExecutor<ToolInvocation> for UpdateGoalHandler {
    ...
    async fn handle(
        ...
        if !matches!(
            ...
            return Err(FunctionCallError::RespondToModel(
                "update_goal can only mark the existing goal complete, blocked, usage-limited, or budget-limited; pause and resume status changes are controlled by the user or system"
                    .to_string(),
            ));
        }
        session
