...
impl ApplyPatchRuntime {
    ...
    pub fn build_sandbox_command(
        ...
        let codex_exe = req.codex_exe.as_ref().or(codex_self_exe);
        Self::build_command_spec(req, std::path::Path::new("."), codex_exe)
    }
    ...
    fn build_command_spec(
        ...
        let exe = if let Some(path) = codex_exe {
            path.clone()
        } else {
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        let spec =
            Self::build_command_spec(req, &ctx.turn.config.codex_home, req.codex_exe.as_ref())?;
        let env = attempt
