...
use codex_api::parse_promo_message;
use codex_api::parse_rate_limit_for_limit;
                ...
                if status == http::StatusCode::BAD_REQUEST {
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return CodexErr::UsageLimitReached(UsageLimitReachedError::new(
                                err.error.plan_type,
                                resets_at,
                                rate_limits.map(Box::new),
                                promo_message,
                            ));
    ...
    let api_key = match provider.api_key() {
        ...
        Err(err) => {
            return Err(match err {
                codex_protocol::error::CodexErr::EnvVar(env_var) => {
                    CodexErr::EnvVar(crate::error::EnvVarError {
                        var: env_var.var,
                        instructions: env_var.instructions,
                    })
                }
