...
mod seccompiler {
    ...
    #[derive(Debug, Error)]
    #[error("seccompiler backend error")]
...
impl From<codex_protocol::error::CodexErr> for CodexErr {
    fn from(err: codex_protocol::error::CodexErr) -> Self {
        match err {
            codex_protocol::error::CodexErr::TurnAborted => CodexErr::TurnAborted,
            codex_protocol::error::CodexErr::Stream(msg, delay) => CodexErr::Stream(msg, delay),
            codex_protocol::error::CodexErr::ContextWindowExceeded => {
                CodexErr::ContextWindowExceeded
            }
            codex_protocol::error::CodexErr::ThreadNotFound(thread_id) => {
                CodexErr::ThreadNotFound(thread_id)
            }
            codex_protocol::error::CodexErr::AgentLimitReached { max_threads } => {
                CodexErr::AgentLimitReached { max_threads }
            }
            codex_protocol::error::CodexErr::SessionConfiguredNotFirstEvent => {
                CodexErr::SessionConfiguredNotFirstEvent
            }
            codex_protocol::error::CodexErr::Timeout => CodexErr::Timeout,
            codex_protocol::error::CodexErr::Spawn => CodexErr::Fatal("spawn failed".to_string()),
            codex_protocol::error::CodexErr::Interrupted => CodexErr::Interrupted,
            codex_protocol::error::CodexErr::UnexpectedStatus(err) => {
                CodexErr::UnexpectedStatus(UnexpectedResponseError {
                    ...
                    identity_authorization_error: err.identity_authorization_error,
                    identity_error_code: err.identity_error_code,
                })
            }
            codex_protocol::error::CodexErr::InvalidRequest(msg) => CodexErr::InvalidRequest(msg),
            codex_protocol::error::CodexErr::InvalidImageRequest() => {
                CodexErr::InvalidImageRequest()
            }
            codex_protocol::error::CodexErr::UsageLimitReached(_) => {
                CodexErr::UsageLimitReached(UsageLimitReachedError::new(None, None, None, None))
            }
            codex_protocol::error::CodexErr::ServerOverloaded => CodexErr::ServerOverloaded,
            codex_protocol::error::CodexErr::ResponseStreamFailed(err) => {
                CodexErr::ResponseStreamFailed(ResponseStreamFailed {
                    source: err.source,
                    request_id: err.request_id,
                })
            }
            codex_protocol::error::CodexErr::ConnectionFailed(err) => {
                CodexErr::ConnectionFailed(ConnectionFailedError { source: err.source })
            }
            codex_protocol::error::CodexErr::QuotaExceeded => CodexErr::QuotaExceeded,
            codex_protocol::error::CodexErr::UsageNotIncluded => CodexErr::UsageNotIncluded,
            codex_protocol::error::CodexErr::InternalServerError => CodexErr::InternalServerError,
            codex_protocol::error::CodexErr::RetryLimit(err) => {
                CodexErr::RetryLimit(RetryLimitReachedError {
                    status: err.status,
                    request_id: err.request_id,
                })
            }
            codex_protocol::error::CodexErr::InternalAgentDied => CodexErr::InternalAgentDied,
            codex_protocol::error::CodexErr::Sandbox(err) => CodexErr::Sandbox(match err {
                codex_protocol::error::SandboxErr::Denied {
                    ...
                    output: Box::new(ExecToolCallOutput {
                        ...
                        stdout: crate::exec::StreamOutput {
                            text: output.stdout.text,
                            truncated_after_lines: output.stdout.truncated_after_lines,
                        },
                        stderr: crate::exec::StreamOutput {
                            text: output.stderr.text,
                            truncated_after_lines: output.stderr.truncated_after_lines,
                        },
                        aggregated_output: crate::exec::StreamOutput {
                            text: output.aggregated_output.text,
                            truncated_after_lines: output.aggregated_output.truncated_after_lines,
                        },
                ...
                #[cfg(target_os = "linux")]
                codex_protocol::error::SandboxErr::SeccompInstall(_) => {
                    SandboxErr::SeccompInstall(seccompiler::Error)
                }
                #[cfg(target_os = "linux")]
                codex_protocol::error::SandboxErr::SeccompBackend(_) => {
                    SandboxErr::SeccompBackend(seccompiler::BackendError)
                }
                codex_protocol::error::SandboxErr::Timeout { output } => SandboxErr::Timeout {
                    output: Box::new(ExecToolCallOutput {
                        exit_code: output.exit_code,
                        stdout: crate::exec::StreamOutput {
                            text: output.stdout.text,
                ...
                codex_protocol::error::SandboxErr::Signal(signal) => SandboxErr::Signal(signal),
                codex_protocol::error::SandboxErr::LandlockRestrict => SandboxErr::LandlockRestrict,
            }),
            codex_protocol::error::CodexErr::LandlockSandboxExecutableNotProvided => {
                CodexErr::LandlockSandboxExecutableNotProvided
            }
            codex_protocol::error::CodexErr::UnsupportedOperation(msg) => {
                CodexErr::UnsupportedOperation(msg)
            }
            codex_protocol::error::CodexErr::RefreshTokenFailed(err) => {
                CodexErr::RefreshTokenFailed(err)
            }
            codex_protocol::error::CodexErr::Fatal(msg) => CodexErr::Fatal(msg),
            codex_protocol::error::CodexErr::Io(err) => CodexErr::Io(err),
            codex_protocol::error::CodexErr::Json(err) => CodexErr::Json(err),
            #[cfg(target_os = "linux")]
            codex_protocol::error::CodexErr::LandlockRuleset(_) => {
                CodexErr::LandlockRuleset(landlock::RulesetError)
            }
            #[cfg(target_os = "linux")]
            codex_protocol::error::CodexErr::LandlockPathFd(_) => {
                CodexErr::LandlockPathFd(landlock::PathFdError)
            }
            codex_protocol::error::CodexErr::TokioJoin(err) => CodexErr::TokioJoin(err),
            codex_protocol::error::CodexErr::EnvVar(err) => CodexErr::EnvVar(EnvVarError {
                var: err.var,
