...
impl Codex {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        let session_configuration = SessionConfiguration {
            ...
            permission_profile_state: PermissionProfileState::from_constrained_active_profile(
    ...
    pub async fn submit_user_input_with_client_user_message_id(
        ...
        self.submit_with_trace(op, trace).await
    }
    ...
    pub async fn preview_settings(
        ...
        state
            .session_configuration
            .apply(updates)
            .map(|config| config.thread_config_snapshot())
    }
...
impl Session {
    ...
    pub(crate) async fn turn_context_for_sub_id(&self, sub_id: &str) -> Option<Arc<TurnContext>> {
        ...
        active.as_ref().and_then(|turn| {
            turn.task
                ...
                .then(|| Arc::clone(&turn.task.as_ref().unwrap().turn_context))
        })
    }
    ...
    pub async fn auto_compact_window_snapshot(&self) -> crate::state::AutoCompactWindowSnapshot {
        self.state.lock().await.auto_compact_window_snapshot()
    }
    ...
    pub async fn steer_input(
        ...
        let active_turn_id = &task.turn_context.sub_id;
        ...
        match active_turn.task.as_ref().map(|task| task.kind) {
        ...
        if let Some(responsesapi_client_metadata) = responsesapi_client_metadata
            && let Some(active_task) = active_turn.task.as_ref()
        {
            active_task
                .turn_context
                .turn_metadata_state
    ...
    pub async fn inject_response_items(
        ...
        match active.as_mut() {
            Some(at) => {
                let mut ts = at.turn_state.lock().await;
                for item in input {
                    ts.push_pending_input(TurnInput::ResponseItem(ResponseItem::from(item)));
                }
                Ok(())
            }
            None => Err(input),
    ...
    pub(crate) async fn subscribe_mailbox_seq(&self) -> watch::Receiver<()> {
