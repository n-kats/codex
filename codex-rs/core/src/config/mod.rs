...
pub(crate) const DEFAULT_MULTI_AGENT_V2_DEFAULT_WAIT_TIMEOUT_MS: i64 =
    DEFAULT_MULTI_AGENT_V2_MIN_WAIT_TIMEOUT_MS;
pub(crate) const DEFAULT_MULTI_AGENT_V2_MAX_WAIT_TIMEOUT_MS: i64 =
    MAX_MULTI_AGENT_V2_WAIT_TIMEOUT_MS;
pub(crate) const HARD_MAX_MULTI_AGENT_V2_TIMEOUT_MS: i64 = MAX_MULTI_AGENT_V2_WAIT_TIMEOUT_MS;
...
impl Config {
    ...
    pub fn tui_pet(&self) -> Option<String> {
        self.config_layer_stack
            .effective_config()
            .as_table()
            .and_then(|table| table.get("tui"))
            .and_then(TomlValue::as_table)
            .and_then(|tui| tui.get("pet"))
            .and_then(TomlValue::as_str)
            .map(str::to_string)
    }

    pub fn tui_pet_anchor(&self) -> codex_config::types::TuiPetAnchor {
        match self
            .config_layer_stack
            .effective_config()
            .as_table()
            .and_then(|table| table.get("tui"))
            .and_then(TomlValue::as_table)
            .and_then(|tui| tui.get("pet_anchor"))
            .and_then(TomlValue::as_str)
        {
            Some("screen-bottom") => codex_config::types::TuiPetAnchor::ScreenBottom,
            Some("composer") | None => codex_config::types::TuiPetAnchor::Composer,
            Some(_) => codex_config::types::TuiPetAnchor::Composer,
        }
    }
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        if overrides.strict_config {
            for layer in config_layer_stack.get_layers(
                ...
                let contents = if let Some(raw_toml) = layer.raw_toml() {
                    raw_toml.to_string()
                } else if matches!(layer.name, ConfigLayerSource::SessionFlags) {
                    toml::to_string(&layer.config)
                        .map_err(|err| std::io::Error::new(ErrorKind::InvalidData, err))?
                } else {
                    continue;
                };

                let validation_path = match &layer.name {
                    ...
                    ConfigLayerSource::Project { dot_codex_folder } => {
                        dot_codex_folder.join(CONFIG_TOML_FILE).to_path_buf()
                    }
                    ConfigLayerSource::LegacyManagedConfigTomlFromMdm => {
                        PathBuf::from("managed_config.toml")
                    }
                    ConfigLayerSource::Mdm { .. } => PathBuf::from("mdm-config.toml"),
                    ConfigLayerSource::SessionFlags => {
                        PathBuf::from("-c/--config override")
                    }
                };

                if let Some(config_error) = config_error_from_ignored_toml_fields::<ConfigToml>(
                    &validation_path,
                    &contents,
                ) {
                    return Err(codex_config::io_error_from_config_error(
        ...
        let include_apply_patch_tool =
            include_apply_patch_tool_override.unwrap_or(features.enabled(Feature::ApplyPatchFreeform));
        let use_experimental_unified_exec_tool = features.enabled(Feature::UnifiedExec);

        let forced_chatgpt_workspace_id =
            cfg.forced_chatgpt_workspace_id.clone().map(|value| value.into_vec());
        ...
        let service_tier = match service_tier.as_deref().and_then(ServiceTier::from_request_value)
        {
            Some(ServiceTier::Fast) if features.enabled(Feature::FastMode) => Some(ServiceTier::Fast),
        ...
        let config = Self {
            ...
            otel: {
                ...
                OtelConfig {
                    ...
                    trace_exporter,
                    metrics_exporter,
                    span_attributes: t.span_attributes.unwrap_or_default(),
                    tracestate: t.tracestate.unwrap_or_default(),
                }
            },
        };
        Ok(config)
