...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    if !json_mode && let Some(message) = codex_core::config::system_bwrap_warning() {
        let _ = event_processor.process_warning(message);
    }
...
fn local_external_chatgpt_tokens(
    ...
    let chatgpt_plan_type = auth.account_plan_type().map(|plan_type| match plan_type {
        ...
        AccountPlanType::SelfServeBusinessUsageBased => "business".to_string(),
        AccountPlanType::EnterpriseCbpUsageBased => "enterprise".to_string(),
