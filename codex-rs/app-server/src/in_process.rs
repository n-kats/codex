...
fn start_uninitialized(args: InProcessStartArgs) -> InProcessClientHandle {
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let mut processor_handle = tokio::spawn(async move {
            let state_db = init_state_db(args.config.as_ref()).await;
            let installation_id = resolve_installation_id(&args.config.codex_home)
                .await
                .unwrap_or_else(|err| {
                    warn!(
                        error = %err,
                        "failed to resolve installation id for in-process app-server"
                    );
                    "unknown".to_string()
                });
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "in-process runtime currently overflows the stack; covered by other app-server tests"]
    ...
    #[tokio::test]
    #[ignore = "in-process runtime currently overflows the stack; covered by other app-server tests"]
    async fn in_process_handles_config_read_request() {
        let client = start_test_client(SessionSource::Cli).await;
        let response = client
            .request(ClientRequest::ConfigRequirementsRead {
    ...
    #[tokio::test]
    #[ignore = "in-process thread/start currently overflows the stack; covered by other app-server tests"]
    ...
    #[tokio::test]
    #[ignore = "in-process start currently overflows the stack; covered by other app-server tests"]
