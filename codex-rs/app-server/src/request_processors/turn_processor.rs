...
impl TurnRequestProcessor {
    ...
    pub(crate) async fn thread_settings_update(
        ...
        let thread_settings = codex_protocol::protocol::ThreadSettingsOverrides {
            ...
            approval_policy: params.approval_policy.map(|value| value.to_core()),
            approvals_reviewer: params.approvals_reviewer.map(|value| value.to_core()),
            sandbox_policy: params.sandbox_policy.map(|value| value.to_core()),
        ...
        Ok(Some(ThreadSettingsUpdateResponse {}.into()))
    }
