...
pub enum Op {
    ...
    /// List custom prompt files discovered for the current Codex home and additional prompt
    /// directories.
    ListCustomPrompts,
...
pub enum EventMsg {
    ...
    /// Realtime conversation SDP answer for WebRTC startup.
    ...
    fn item_completed_event_from_image_generation_emits_end_event() {
        let event = ItemCompletedEvent {
            ...
            item: TurnItem::ImageGeneration(ImageGenerationItem {
                ...
                result: "Zm9v".into(),
                saved_path: Some(
                    codex_utils_absolute_path::AbsolutePathBuf::try_from("/tmp/ig-1.png").unwrap(),
                ),
            }),
        };
        ...
        match &legacy_events[0] {
            EventMsg::ImageGenerationEnd(event) => {
                ...
                assert_eq!(
                    event.saved_path,
                    Some(
                        codex_utils_absolute_path::AbsolutePathBuf::try_from("/tmp/ig-1.png")
                            .unwrap(),
                    )
                );
            }
