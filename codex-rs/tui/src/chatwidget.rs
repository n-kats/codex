...
pub(crate) struct ChatWidget {
    ...
    // When set, the first `/compact` submission is pending its turn start and
    // follow-up input should be queued instead of submitted immediately.
    compact_turn_pending_start: bool,
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: codex_protocol::protocol::SessionConfiguredEvent) {
        ...
        // If the user typed while the session was still initializing, try to
        // flush any queued follow-ups now that the thread is ready.
    ...
    fn show_feedback_note(
        ...
        category: crate::app_event::FeedbackCategory,
        include_logs: bool,
        _snapshot: codex_feedback::FeedbackSnapshot,
    ) {
    ...
    fn on_task_started(&mut self) {
        ...
        self.compact_turn_pending_start = false;
    ...
    pub(crate) fn update_account_state(
        ...
        account_display: Option<StatusAccountDisplay>,
        plan_type: Option<PlanType>,
        _has_chatgpt_account: bool,
    ) {
        self.account_display = account_display;
        self.plan_type = plan_type.or(self.plan_type);
        self.refresh_status_surfaces();
    }
    ...
    pub(crate) fn handle_server_notification(
        ...
        match notification {
            ServerNotification::AccountRateLimitsUpdated(notification) => {
                self.on_rate_limit_snapshot(Some(app_server_rate_limit_snapshot_to_core(
    ...
    /// Fold an MCP startup status update into the active round, or buffer it until we can safely
    /// treat it as a fresh startup round.
    ...
    fn on_mcp_startup_complete(&mut self, ev: McpStartupCompleteEvent) {
        ...
        let failed_servers: Vec<String> = ev.failed.into_iter().map(|f| f.server).collect();
        self.finish_mcp_startup(failed_servers, ev.cancelled);
        ...
        let guardian_action_summary = |action: &GuardianAssessmentAction| match action {
            ...
            GuardianAssessmentAction::ApplyPatch { files, .. } => {
                if files.len() == 1 {
                    ...
                    Some(format!("apply_patch touching {} files", files.len()))
                }
            }
            ...
            GuardianAssessmentAction::McpToolCall {
                ...
                "MCP {tool_name} on {}",
                connector_name.as_deref().unwrap_or(server.as_str())
            )),
        };
        ...
        let cell = if let Some(command) = guardian_command(&action) {
            ...
            match action {
                ...
                | GuardianAssessmentAction::Execve { .. } => {
                    history_cell::new_guardian_denied_action_request(format!("{action:?}"))
                }
            }
        };
    ...
    fn on_image_generation_end(&mut self, event: ImageGenerationEndEvent) {
        ...
        let saved_path = event.saved_path.map(|saved_path| {
            Url::from_file_path(saved_path.as_path())
                ...
                .unwrap_or_else(|_| saved_path.as_path().to_string_lossy().to_string())
        });
    ...
    pub(crate) fn handle_apply_patch_approval_now(&mut self, ev: ApplyPatchApprovalRequestEvent) {
        ...
        let request = ApprovalRequest::ApplyPatch {
            ...
            cwd: self.config.cwd.clone().try_into().expect("absolute cwd"),
        };
        self.bottom_pane
            .push_approval_request(request, &self.config.features);
        self.request_redraw();
    ...
    fn new_with_transport(common: ChatWidgetInit, command_transport: CommandTransport) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
            app_event_tx,
        ...
        let mut widget = Self {
            ...
            compact_turn_pending_start: false,
    ...
    pub(crate) fn new_with_op_sender(
        ...
        Self::new_with_transport(common, CommandTransport::Direct(codex_op_tx))
    }

    pub(crate) fn new_with_app_event(common: ChatWidgetInit) -> Self {
        Self::new_with_transport(common, CommandTransport::AppEvent)
    }

    pub(crate) fn new_with_disabled_transport(common: ChatWidgetInit) -> Self {
        Self::new_with_transport(common, CommandTransport::Disabled)
    }
    ...
    pub(crate) fn new_from_existing(
        ...
        let command_transport = CommandTransport::AppEvent;
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.compact_turn_pending_start = true;
                if !self.bottom_pane.is_task_running() {
                    self.bottom_pane.set_task_running(/*running*/ true);
                }
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        if !text.is_empty() {
            ...
            if !self.submit_op(Op::AddToHistory { text: history_text }) {
                tracing::error!("failed to send AddHistory op");
            }
        }
    ...
    fn connectors_enabled(&self) -> bool {
        self.config
            .features
            .apps_enabled_cached(self.auth_manager.auth_cached().is_some())
    }
    ...
    pub(crate) fn add_mcp_output(&mut self) {
        ...
        let effective_servers = tokio::runtime::Handle::current()
            .block_on(mcp_manager.effective_servers(&self.config, /*auth*/ None));
    ...
    #[cfg(test)]
    pub(crate) fn push_queued_user_message_for_test(&mut self, user_message: UserMessage) {
        self.queued_user_messages.push_back(user_message);
        self.refresh_pending_input_preview();
    }
    ...
    /// Forward an `Op` to the active session transport.
    pub(crate) fn submit_op(&mut self, op: Op) -> bool {
        ...
        match &self.command_transport {
            #[cfg(test)]
            CommandTransport::Direct(codex_op_tx) => {
                if let Err(e) = codex_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            CommandTransport::AppEvent => {
    ...
    fn on_list_mcp_tools(&mut self, ev: McpListToolsResponseEvent) {
        if self.connectors_enabled() {
            let mut connectors_by_id: HashMap<String, AppInfo> = HashMap::new();
    ...
    fn refresh_plugin_mentions(&mut self) {
        ...
        let plugins = tokio::runtime::Handle::current().block_on(
            PluginsManager::new(self.config.codex_home.clone()).plugins_for_config(&self.config),
        );
        let plugins = plugins.capability_summaries().to_vec();
