...
pub(crate) use rate_limits::fallback_limit_label;
...
pub(crate) struct ChatWidget {
    ...
    config: Config,
    tui_pet: Option<String>,
    tui_pet_anchor: TuiPetAnchor,
    ...
    status_state: status_state::StatusState,
    ...
    // Cached git summary for the status line.
...
impl ChatWidget {
    ...
    pub(crate) fn history_render_mode(&self) -> crate::history_cell::HistoryRenderMode {
    ...
    fn new_with_op_target(common: ChatWidgetInit, codex_op_target: CodexOpTarget) -> Self {
        ...
        let raw_output_mode = config.tui_raw_output_mode;
        ...
        let transcript_active_cell = Some(Self::placeholder_session_header_cell(&config));
        ...
        let mut widget = Self {
            ...
            transcript: transcript::TranscriptState::new(transcript_active_cell),
    ...
    fn approval_preset_actions(
        ...
        active_permission_profile: codex_protocol::models::ActivePermissionProfile,
        label: String,
    ...
    pub(crate) fn toggle_fast_mode_from_ui(&mut self) {
        ...
        let next_tier = if self
            ...
            .is_some_and(|tier| tier.request_value() == fast_tier.id)
        {
            None
        } else {
    ...
    pub(crate) fn toggle_service_tier_from_ui(&mut self, command: ServiceTierCommand) {
        let next_tier = if self
            .current_service_tier()
            .is_some_and(|tier| tier.request_value() == command.id)
        {
            None
        } else {
            ServiceTier::from_request_value(&command.id)
        };
        self.set_service_tier_selection(next_tier);
    }
    ...
    fn current_model_fast_service_tier(&self) -> Option<ServiceTierCommand> {
        self.current_model_service_tier_commands()
            ...
            .find(|tier| {
                tier.name
                    .eq_ignore_ascii_case(codex_protocol::openai_models::SPEED_TIER_FAST)
            })
    }
    ...
    fn set_service_tier_selection(&mut self, service_tier: Option<ServiceTier>) {
        ...
        let service_tier_for_event = service_tier.clone();
        self.app_event_tx
        ...
        self.app_event_tx
            .send(AppEvent::PersistServiceTierSelection {
                service_tier: service_tier_for_event,
            });
    }
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = codex_utils_cli::resume_command(Some(name), thread_id)
...
impl ChatWidget {
    pub(crate) fn set_queue_submissions_until_session_configured(&mut self, queue: bool) {
        self.set_queue_autosend_suppressed(queue && !self.is_session_configured());
    }

    pub(crate) fn on_thread_settings_updated(
        &mut self,
        notification: codex_app_server_protocol::ThreadSettingsUpdatedNotification,
    ) {
