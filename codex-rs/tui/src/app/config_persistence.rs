...
impl App {
    fn current_user_config_path(&self) -> Option<AbsolutePathBuf> {
        let layer = self.config.config_layer_stack.get_active_user_layer()?;
    ...
    pub(super) fn apply_runtime_policy_overrides(&mut self, config: &mut Config) {
        ...
        if let Some(profile) = self.runtime_permission_profile_override.as_ref()
            && let Err(err) = config
                .permissions
                .set_permission_profile(profile.permission_profile.clone())
        {
    ...
    pub(super) fn try_set_builtin_active_permission_profile_on_config(
        ...
        let Some(permission_profile) =
            ...
            tracing::warn!(
                error = %active_permission_profile.id,
                "{log_message}"
            );
            self.chat_widget.add_error_message(format!(
                "{user_message_prefix}: unsupported permission profile"
            ));
            return None;
        };
    ...
    pub(super) async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        for (feature, enabled) in updates {
            ...
            if feature == Feature::GuardianApproval
                && !enabled
                && self.config.active_profile.is_some()
            ...
            if feature == Feature::GuardianApproval && effective_enabled {
                ...
                let Some(_permission_profile) = self
