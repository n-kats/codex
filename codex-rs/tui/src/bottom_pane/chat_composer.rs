...
impl ChatComposer {
        ...
        let in_slash_context = self.slash_commands_enabled()
            && (matches!(self.active_popup, ActivePopup::Command(_))
                || self
                    ...
                    .unwrap_or("")
                    .starts_with('/'));
        if let Some(result) = self.try_dispatch_pending_bare_slash_command() {
            return (result, true);
        }
    ...
    /// Check whether the current paste-burst state already contains a bare
    /// slash command that should be dispatched immediately.
    fn try_dispatch_pending_bare_slash_command(&mut self) -> Option<InputResult> {
        if !self.slash_commands_enabled() {
            return None;
        }
        let pending_text = self.paste_burst.pending_text()?;
        let first_line = pending_text.lines().next().unwrap_or("");
        if let Some((name, rest, _rest_offset)) = parse_slash_name(first_line)
            && rest.is_empty()
            && let Some(cmd) =
                slash_commands::find_builtin_command(name, self.builtin_command_flags())
