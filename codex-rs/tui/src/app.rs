...
impl ThreadEventStore {
    ...
    fn apply_thread_rollback(&mut self, _response: &ThreadRollbackResponse) {
        self.buffer.clear();
...
fn list_skills_response_to_core(response: SkillsListResponse) -> ListSkillsResponseEvent {
    let skills = response
        ...
        .into_iter()
        .filter_map(|entry| {
            serde_json::from_value::<codex_protocol::protocol::SkillsListEntry>(
...
pub(crate) struct App {
    ...
    current_model_reasoning_effort: Option<ReasoningEffortConfig>,
...
impl App {
    ...
    async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        ConfigBuilder::default()
            ...
            .await
            .map(|mut config| {
                config.model_reasoning_effort = self.current_model_reasoning_effort;
                config
            })
    ...
    fn fetch_plugins_list(&mut self, app_server: &AppServerSession, cwd: PathBuf) {
        ...
        tokio::spawn(async move {
            ...
            let result = match AbsolutePathBuf::try_from(cwd) {
                Ok(cwd) => request_handle
                    .request_typed(ClientRequest::PluginList {
                        request_id: RequestId::String(format!("plugin-list-{}", Uuid::new_v4())),
                ...
                Err(err) => Err(format!("Failed to load plugins: {err}")),
            };
        ...
        tokio::spawn(async move {
            ...
            let result = request_handle
                .request_typed(ClientRequest::PluginRead {
                    request_id: RequestId::String(format!("plugin-read-{}", Uuid::new_v4())),
                    params,
                })
                .await
        ...
        tokio::spawn(async move {
            ...
            let result = request_handle
                .request_typed(ClientRequest::PluginInstall {
                    request_id: RequestId::String(format!("plugin-install-{}", Uuid::new_v4())),
        ...
        tokio::spawn(async move {
            ...
            let result = request_handle
                .request_typed(ClientRequest::PluginUninstall {
                    request_id: RequestId::String(format!("plugin-uninstall-{}", Uuid::new_v4())),
    ...
    async fn submit_thread_op(
        ...
        if self
            .try_submit_active_thread_op_via_app_server(app_server, thread_id, &app_command)
            .await?
        {
            if ThreadEventStore::op_can_change_pending_replay_state(&op) {
                self.note_thread_outbound_op(thread_id, &op).await;
    ...
    async fn enqueue_server_notification_events(
        &mut self,
        thread_id: ThreadId,
        events: Vec<Event>,
    ) -> Result<()> {
    ...
    async fn handle_app_server_event(
        ...
        match event {
            ...
            AppServerEvent::ServerNotification(notification) => {
                match &notification {
                    ServerNotification::ThreadStarted(notification) => {
                        let Ok(thread_id) = ThreadId::from_string(&notification.thread.id) else {
                            tracing::warn!(
                                thread_id = %notification.thread.id,
                                "ignoring app-server thread-started notification with invalid thread id"
                            );
                            return Ok(());
                        };
                        if let Some(inferred_session) =
                            self.infer_session_for_thread_started(thread_id, &notification.thread)
                        {
                            let channel = self.ensure_thread_channel(thread_id);
                            let mut store = channel.store.lock().await;
                            if store.session_configured.is_none() {
                                store.session_configured = Some(inferred_session);
                            }
                        }
                    ...
                    ServerNotification::ThreadArchived(notification) => {
                        let Ok(thread_id) = ThreadId::from_string(&notification.thread_id) else {
                            tracing::warn!(
                                ...
                                "ignoring app-server thread-archived notification with invalid thread id"
                            );
                            return Ok(());
                        };
                    ...
                    ServerNotification::ThreadClosed(notification) => {
                        let Ok(thread_id) = ThreadId::from_string(&notification.thread_id) else {
                            tracing::warn!(
                                thread_id = %notification.thread_id,
                                "ignoring app-server thread-closed notification with invalid thread id"
                            );
                            return Ok(());
                        };
                        self.mark_agent_picker_thread_closed(thread_id);
                    ...
                    ServerNotification::ThreadUnarchived(notification) => {
                        let Ok(thread_id) = ThreadId::from_string(&notification.thread_id) else {
                            tracing::warn!(
                                thread_id = %notification.thread_id,
                                "ignoring app-server thread-unarchived notification with invalid thread id"
                            );
                            return Ok(());
                        };
                    ...
                    ServerNotification::ThreadStatusChanged(notification) => {
                        let Ok(thread_id) = ThreadId::from_string(&notification.thread_id) else {
                            tracing::warn!(
                                thread_id = %notification.thread_id,
                                "ignoring app-server thread-status-changed notification with invalid thread id"
                            );
                            return Ok(());
                        };
                        let (agent_nickname, agent_role) = self
                            ...
                            .map(|entry| (entry.agent_nickname.clone(), entry.agent_role.clone()))
                            .unwrap_or((None, None));
                        let is_closed = matches!(notification.status, ThreadStatus::NotLoaded);
                        self.upsert_agent_picker_thread(
                            thread_id,
                            agent_nickname,
                            agent_role,
                    ...
                    ServerNotification::AccountRateLimitsUpdated(notification) => {
                        self.chat_widget.on_rate_limit_snapshot(Some(
                            app_server_rate_limit_snapshot_to_core(
                ...
                if let Some((thread_id, events)) =
                    server_notification_thread_events(notification.clone())
                {
                    self.enqueue_server_notification_events(thread_id, events)
                        .await?;
                } else {
                    self.chat_widget
            ...
            AppServerEvent::ServerRequest(request) => {
                self.handle_app_server_request(app_server, request).await?;
            }
    ...
    fn infer_session_for_thread_started(
        &self,
        thread_id: ThreadId,
        thread: &codex_app_server_protocol::Thread,
    ) -> Option<Event> {
        let mut session = self.primary_session_configured.as_ref()?.clone();
        session.session_id = thread_id;
        session.forked_from_id = match &thread.source {
    ...
    async fn handle_app_server_request(
            ...
            .note_server_request(&request)
        {
            ...
            self.reject_app_server_request(app_server, unsupported.request_id, unsupported.message)
                .await?;
            return Ok(());
        }
        ...
        match request {
            ServerRequest::CommandExecutionRequestApproval { params, .. } => {
                let Ok(thread_id) = ThreadId::from_string(&params.thread_id) else {
                    tracing::warn!(
                        ...
                        "ignoring app-server command approval with invalid thread_id"
                    );
                    return Ok(());
                };
                let thread_label = Some(self.thread_label(thread_id));
                ...
                let available_decisions = params.available_decisions.clone();
                self.chat_widget.push_approval_request(ApprovalRequest::Exec {
                    thread_id,
                    thread_label,
                    id: params.approval_id.unwrap_or_else(|| params.item_id.clone()),
                    command: params
                        .command
                        .as_deref()
                        .map(crate::exec_command::split_command_string)
                        .unwrap_or_default(),
                    reason: params.reason,
                    available_decisions: available_decisions.map_or_else(
                        || {
                            if network_approval_context.is_some() {
                                ...
                                if let Some(amendment) = proposed_network_policy_amendments
                                    .as_ref()
                                    .and_then(|amendments| {
                                        amendments.iter().find(|amendment| {
                                            amendment.action
                                                == codex_app_server_protocol::NetworkPolicyRuleAction::Allow
                                        })
                                    })
                                {
                                    decisions.push(
                                        codex_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                                            network_policy_amendment: amendment.clone().into_core(),
                                        },
                                    );
                                }
                                decisions.push(codex_protocol::protocol::ReviewDecision::Abort);
                                decisions
                                ...
                                let mut decisions =
                                    vec![codex_protocol::protocol::ReviewDecision::Approved];
                                if let Some(prefix) = proposed_execpolicy_amendment.as_ref() {
                                    decisions.push(
                                        codex_protocol::protocol::ReviewDecision::ApprovedExecpolicyAmendment {
                                            proposed_execpolicy_amendment: prefix.clone().into_core(),
                                        },
                                    );
                                }
                                decisions.push(codex_protocol::protocol::ReviewDecision::Abort);
                        ...
                        |decisions| {
                            decisions
                                .into_iter()
                                .map(Self::command_execution_approval_decision_to_review_decision)
                                .collect()
                        },
                    ),
                    network_approval_context: network_approval_context.map(|context| {
                        codex_protocol::protocol::NetworkApprovalContext {
                            ...
                            protocol: match context.protocol {
                                codex_app_server_protocol::NetworkApprovalProtocol::Http => {
                                    codex_protocol::protocol::NetworkApprovalProtocol::Http
                                }
                                codex_app_server_protocol::NetworkApprovalProtocol::Https => {
                                    codex_protocol::protocol::NetworkApprovalProtocol::Https
                                }
                                codex_app_server_protocol::NetworkApprovalProtocol::Socks5Tcp => {
                                    codex_protocol::protocol::NetworkApprovalProtocol::Socks5Tcp
                                }
                                codex_app_server_protocol::NetworkApprovalProtocol::Socks5Udp => {
                                    codex_protocol::protocol::NetworkApprovalProtocol::Socks5Udp
                                }
                            },
                        }
                    }),
            ...
            ServerRequest::FileChangeRequestApproval { params, .. } => {
                let Ok(thread_id) = ThreadId::from_string(&params.thread_id) else {
                    tracing::warn!(
                        thread_id = %params.thread_id,
                        "ignoring app-server file change approval with invalid thread_id"
                    );
                    return Ok(());
                };
                let thread_label = Some(self.thread_label(thread_id));
                self.chat_widget
                    .push_approval_request(ApprovalRequest::ApplyPatch {
                        thread_id,
                        thread_label,
                        id: params.item_id,
                        reason: params.reason,
            ...
            ServerRequest::PermissionsRequestApproval { params, .. } => {
                let Ok(thread_id) = ThreadId::from_string(&params.thread_id) else {
                    tracing::warn!(
                        thread_id = %params.thread_id,
                        "ignoring app-server permissions approval with invalid thread_id"
                    );
                    return Ok(());
                };
                let thread_label = Some(self.thread_label(thread_id));
            ...
            ServerRequest::McpServerElicitationRequest { request_id, params } => {
                let Ok(thread_id) = ThreadId::from_string(&params.thread_id) else {
                    tracing::warn!(
                        thread_id = %params.thread_id,
                        "ignoring app-server MCP elicitation with invalid thread_id"
                    );
                    return Ok(());
                };
                let request_id = Self::app_server_request_id_to_mcp_request_id(&request_id);
                if let Some(request) = McpServerElicitationFormRequest::from_app_server_request(
                    ...
                    self.chat_widget
                        .push_approval_request(ApprovalRequest::McpElicitation {
                            thread_id,
                            thread_label: Some(self.thread_label(thread_id)),
                            server_name: params.server_name,
                            request_id,
                            message: Self::mcp_server_elicitation_request_message(&params.request)
                                .to_string(),
                        });
                }
            }
    ...
    async fn reject_app_server_request(
        ...
        app_server
            ...
            .await
            .map_err(|err| color_eyre::eyre::eyre!("failed to reject app-server request: {err}"))?;
        Ok(())
    }
    ...
    fn mcp_server_elicitation_request_message(
    ...
    fn handle_skills_list_response(&mut self, response: SkillsListResponse) {
        ...
        self.chat_widget.handle_codex_event(Event {
            id: String::new(),
            msg: EventMsg::ListSkillsResponse(response),
        });
    }
    ...
    async fn enqueue_thread_history_entry_response(
        ...
        self.chat_widget.handle_codex_event(Event {
            id: String::new(),
            msg: EventMsg::GetHistoryEntryResponse(event),
        });
        Ok(())
    }
    ...
    fn handle_backtrack_rollback_failed(&mut self) {
        self.backtrack.pending_rollback = None;
        self.chat_widget
            .add_error_message("Backtrack rollback failed.".to_string());
    }
    ...
    async fn select_agent_thread(
        ...
        let is_replay_only = !live_thread;
        ...
        if live_thread {
            ...
            self.replace_chat_widget(ChatWidget::new_with_disabled_transport(init));
        }
    ...
    async fn enqueue_primary_thread_session(
        ...
        self.enqueue_primary_event(Event {
            id: String::new(),
            msg: EventMsg::SessionConfigured(session.to_session_configured_event()),
        })
        .await?;
        for event in app_server_turns_to_events(thread_id, &turns) {
            self.enqueue_thread_event(thread_id, event).await?;
        }
        Ok(())
    }
    ...
    /// Rebuilds the `/agent` picker cache from the app server's currently loaded threads.
    ///
    /// This is used after live thread replacement so already-running subagents remain visible
    /// in the picker even before they emit fresh notifications.
    ...
    pub async fn run(
        ...
        let initial_started_thread = match &session_selection {
            SessionSelection::StartFresh | SessionSelection::Exit => Some(
                app_server
                    .start_thread(&config)
                    .await
                    .wrap_err("failed to start initial thread")?,
            ),
            SessionSelection::Resume(_) | SessionSelection::Fork(_) => None,
        };
        let mut chat_widget = match session_selection {
            SessionSelection::StartFresh | SessionSelection::Exit => {
                initial_started_thread
                    .as_ref()
                    .expect("fresh sessions should start an initial thread");
            ...
            SessionSelection::Resume(target_session) => {
                let Some(target_path) = target_session.path.clone() else {
                    let thread_id = target_session.thread_id.to_string();
                    return Err(color_eyre::eyre::eyre!(
                        "missing rollout path for session {thread_id}"
                    ));
                };
                let resumed = app_server
                    ...
                    .await
                    .wrap_err_with(|| {
                        let path_display = target_path.display();
                ...
                let session_configured = resumed.session.to_session_configured_event();
                let _ = session_configured;
                ChatWidget::new_from_existing(init, resumed.session.to_session_configured_event())
            }
            SessionSelection::Fork(target_session) => {
                ...
                let session_configured = forked.session.to_session_configured_event();
                let _ = session_configured;
                ChatWidget::new_from_existing(init, forked.session.to_session_configured_event())
            }
        };
        ...
        let mut app = Self {
            ...
            auth_manager: auth_manager.clone(),
            config,
            current_model_reasoning_effort,
            active_profile,
            cli_kv_overrides,
            loader_overrides,
            harness_overrides,
                ...
                let control = select! {
                    ...
                    active = async {
                        ...
                        if let Some(event) = active {
                            ...
                            if app.pending_shutdown_exit_thread_id.is_some() {
                                break Ok(ExitReason::UserRequested);
                            }
                    ...
                    app_server_event = async { app_server.next_event().await }, if listen_for_app_server_events => {
                        match app_server_event {
                    Some(event) => {
                                if let Err(err) = app.handle_app_server_event(&mut app_server, event).await {
                                    break Err(err);
                                }
                            }
                            None => {
    ...
    fn on_update_reasoning_effort(&mut self, effort: Option<ReasoningEffortConfig>) {
        // Keep the canonical effort on `App`; session rebuilds read from this state.
        self.current_model_reasoning_effort = effort;
...
mod tests {
    ...
    async fn app_server_thread_started_updates_picker_and_session_cache() -> Result<()> {
        ...
        app.primary_session_configured = Some(SessionConfiguredEvent {
            ...
            forked_from_id: None,
            thread_name: Some("primary".to_string()),
            model: "gpt-test".to_string(),
            model_provider_id: "test-provider".to_string(),
            service_tier: None,
            approval_policy: AskForApproval::Never,
            ...
            initial_messages: None,
            network_proxy: None,
            rollout_path: Some(PathBuf::from("/tmp/primary-rollout")),
        });

        let thread_id = ThreadId::new();
        let notification = AppServerEvent::ServerNotification(ServerNotification::ThreadStarted(
                ...
                thread: codex_app_server_protocol::Thread {
                    ...
                    source: codex_app_server_protocol::SessionSource::SubAgent(
                        codex_protocol::protocol::SubAgentSource::ThreadSpawn {
                            ...
                            depth: 1,
                            agent_path: None,
                            agent_nickname: Some("buddy".to_string()),
        ...
        let entry = app
            .agent_navigation
            .get(&thread_id)
            .expect("thread should be visible in the picker");
        assert_eq!(entry.agent_nickname.as_deref(), Some("buddy"));
        ...
        let channel = app
            .thread_event_channels
            .get(&thread_id)
            .expect("thread channel should be created");
        let store = channel.store.lock().await;
        let Some(Event {
            msg: EventMsg::SessionConfigured(session),
            ..
            ...
            panic!("thread-started notification should seed a session configured event");
        };
        assert_eq!(session.session_id, thread_id);
        assert_eq!(session.forked_from_id, Some(primary_thread_id));
        assert_eq!(session.thread_name.as_deref(), Some("sub-agent"));
        assert_eq!(session.model_provider_id, "test-provider");
        assert_eq!(session.cwd, PathBuf::from("/tmp/project"));
        assert_eq!(session.history_log_id, 0);
        assert_eq!(session.history_entry_count, 0);
    ...
    async fn replayed_turn_complete_submits_restored_queued_follow_up() {
        ...
        app.chat_widget
            .push_queued_user_message_for_test(UserMessage::from("queued follow-up".to_string()));
        let input_state = app
            .chat_widget
            .capture_thread_input_state()
...
"#;
    ...
    fn next_list_skills_op(op_rx: &mut tokio::sync::mpsc::UnboundedReceiver<Op>) -> Op {
        let mut seen = Vec::new();
        while let Ok(op) = op_rx.try_recv() {
            if matches!(op, Op::ListSkills { .. }) {
                return op;
            }
            seen.push(format!("{op:?}"));
        }
        panic!("expected ListSkills op, saw: {seen:?}");
    }
    ...
    async fn update_reasoning_effort_updates_collaboration_mode() {
        ...
        assert_eq!(
            app.current_model_reasoning_effort,
            Some(ReasoningEffortConfig::High)
        );
    }
