...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let mut w = ChatWidget::new_with_op_sender(init, op_tx);
...
async fn make_chatwidget_manual(
    ...
    let mut widget = ChatWidget {
        app_event_tx,
        command_transport: CommandTransport::Direct(op_tx),
        bottom_pane: bottom,
        active_cell: None,
        ...
        mcp_startup_expected_servers: Vec::new(),
...
#[tokio::test]
async fn queued_message_before_session_configured_flushes_after_session_configured() {
    ...
    let Op::UserTurn { items, .. } = next_submit_op(&mut op_rx).await else {
        panic!("expected queued message to flush after session configuration");
    };
    assert_eq!(
        items,
        vec![UserInput::Text {
...
#[tokio::test]
async fn queued_message_flushes_after_task_complete() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(None).await;
    ...
    let Op::UserTurn { items, .. } = next_submit_op(&mut op_rx).await else {
        panic!("expected queued message to flush after task completion");
    };
    assert_eq!(
        items,
        vec![UserInput::Text {
...
async fn collaboration_modes_defaults_to_code_on_startup() {
    ...
    let (op_tx, _op_rx) = unbounded_channel::<Op>();
    let chat = ChatWidget::new_with_op_sender(init, op_tx);
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Default);
...
#[tokio::test]
async fn slash_exit_requests_exit_while_task_running() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.on_task_started();
    ...
    assert_eq!(chat.bottom_pane.composer_text(), "/exit");
    let result = chat.bottom_pane.handle_key_event(submit_key_event());
    assert!(matches!(result, InputResult::Command(SlashCommand::Exit)));
    chat.dispatch_command(SlashCommand::Exit);
...
#[tokio::test]
async fn ctrl_d_quits_with_queued_messages_visible() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.bottom_pane.set_task_running(true);
    chat.queued_user_messages
        .push_back(UserMessage::from("queued follow-up".to_string()));
    chat.refresh_pending_input_preview();
...
#[tokio::test]
async fn slash_exit_requests_exit_without_waiting_for_paste_flush() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.on_task_started();
    ...
    loop {
        match rx.try_recv() {
            ...
            Ok(AppEvent::Exit(ExitMode::ShutdownFirst)) => break,
...
#[tokio::test]
async fn task_running_plain_char_input_flushes_after_paste_delay() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.on_task_started();
...
async fn plugins_popup_refreshes_installed_counts_after_install() {
    ...
    assert!(
        before.contains("Installed   Press Enter to view plugin details."),
...
fn snapshot_body(snapshot: &str) -> &str {
    snapshot
        ...
        .expect("invalid insta snapshot file")
}
...
async fn approval_modal_exec_snapshot() -> anyhow::Result<()> {
    ...
    let expected = snapshot_body(include_str!(
        "snapshots/codex_tui__chatwidget__tests__approval_modal_exec.snap"
    ));
    assert_eq!(
        terminal.backend().vt100().screen().contents().trim_end(),
        expected.trim_end()
    );
...
async fn approval_modal_exec_without_reason_snapshot() -> anyhow::Result<()> {
    ...
    let expected = snapshot_body(include_str!(
        "snapshots/codex_tui__chatwidget__tests__approval_modal_exec_no_reason.snap"
    ));
    assert_eq!(
        terminal.backend().vt100().screen().contents().trim_end(),
        expected.trim_end()
...
async fn terminal_title_reschedules_spinner_when_title_text_is_unchanged() {
    ...
    assert!(
        tokio::time::timeout(std::time::Duration::from_secs(1), frame_schedule_rx.recv())
            .await
            .is_ok()
    );
}
...
"#;
    ...
    let expected = snapshot_body(include_str!(
        "snapshots/codex_tui__chatwidget__tests__chatwidget_markdown_code_blocks_vt100_snapshot.snap"
    ));
    assert_eq!(
        term.backend().vt100().screen().contents().trim_end(),
        expected.trim_end()
    );
}
...
async fn review_queues_user_messages_snapshot() {
    ...
    .unwrap();
    let expected = snapshot_body(include_str!(
        "snapshots/codex_tui__chatwidget__tests__review_queues_user_messages_snapshot.snap"
    ));
    assert_eq!(
        term.backend().vt100().screen().contents().trim_end(),
        expected.trim_end()
...
async fn compact_queues_user_messages_snapshot() {
    ...
    .unwrap();
    let expected = snapshot_body(include_str!(
        "snapshots/codex_tui__chatwidget__tests__compact_queues_user_messages_snapshot.snap"
    ));
    assert_eq!(
        term.backend().vt100().screen().contents().trim_end(),
        expected.trim_end()
