...
- 非 release の `cargo build` / `cargo test` は `V8_FROM_SOURCE=0` を既定値とし、利用可能な `rusty_v8` prebuilt artifact を優先する。prebuilt取得に失敗する環境では、`make V8_FROM_SOURCE=1 ...` でsource-build経路を明示的に選ぶ。
- `test-almost` は `V8_FROM_SOURCE`、Cargo の timings 設定、テスト開始・終了時刻を `_tmp/almost_test_result.txt` に記録する。Cargo timings レポートは、Docker runnerが指定するtargetキャッシュ配下に生成される。
- `run_test_logged` は実行コマンドをシェルクォートした単一引数として渡し、リポジトリ内にコマンド受け渡し用の一時ファイルを生成しない。
...
- `Makefile` の `_tmp` はテスト結果ログ専用にし、処理ごとの使い捨て作業領域は `mktemp -d` などの OS 一時ディレクトリを使って終了時に削除する。
