...
- `make release` も同じ artifact と binding を Docker 内の `/workspace/_tmp/assets` から参照する。
...
`make release` を実行する場合も、先に `make download-assets` を実行する。ダウンロード後、Makefile が次の2変数を Cargo に渡すことを確認する。
