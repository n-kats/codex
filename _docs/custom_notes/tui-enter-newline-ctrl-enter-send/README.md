...
## 追記（2026-03-24 rebase 後の補足）

- `/compact` 直後の入力は、既存の `task_running` の扱いに合わせて queue 側へ寄せる。
- いったん専用フラグ案も試したが、`replay` / active turn の期待値を壊すため採用しなかった。
- 結果として、`Enter` の送信判定は元の running 条件に戻し、`/compact` 由来の状態だけを無理に別管理しない。
- 変更ファイル:
  - `codex-rs/tui/src/chatwidget.rs`
  - `codex-rs/tui/src/chatwidget/tests.rs`

## 追記（2026-04-03 StartFresh の送信/終了ハング修正）

- 標準 TUI の `StartFresh` / `Exit` 起動時に、初期 thread は先に作成されていた一方で、`ChatWidget` は `new(...)` 経由で `AppEvent::CodexOp` へ戻す sender を持っていた。
- この状態では、通常送信や `/exit` が初期 thread へ直接 submit されず、`submitted` までは見えても応答や `ShutdownComplete` が返らない症状になりうる。
- 対策として、`StartFresh` / `Exit` でも `ChatWidget::new_from_existing(...)` を使い、起動時に作成した thread へ `Op` を直接 submit するようにした。
- その際、初回バナーと startup tooltip を失わないよう、`new_from_existing(...)` 側でも `is_first_run` / `startup_tooltip_override` を保持するようにした。
- 対象ファイル:
  - `codex-rs/tui/src/app.rs`
  - `codex-rs/tui/src/chatwidget.rs`
  - `codex-rs/tui/src/chatwidget/tests.rs`
- 検証:
  - MCP `cargo_check`
  - MCP `cargo_test_selected`:
    - `shutdown_first_exit_unsubscribes_active_thread_and_exits`
    - `queued_message_before_session_configured_flushes_after_session_configured`
    - `slash_exit_requests_exit_without_waiting_for_paste_flush`
    - `task_running_plain_char_input_flushes_after_paste_delay`

## 追記（2026-04-03 `/new` 後の送信/終了ハング修正）

- 上の `StartFresh` 修正とは別に、`/new` の経路は `start_fresh_session_with_summary_hint()` で依然として `ChatWidget::new(...)` を使っていた。
- そのため `/new` 後は、起動時と同じ種類の不整合が再発しうる。
  - 通常送信は `submitted` まで進んでも実 thread に結線されない。
  - `/exit` は切り替え中の thread を外しきれず、終了直前の状態が崩れやすい。
- 対策として、`/new` でも `server.start_thread(...)` で先に thread を作成し、`ChatWidget::new_from_existing(...)` に切り替えた。
- さらに `handle_thread_created(...)` と `SessionConfigured` の注入も startup と同じ順で行うようにした。
- 対象ファイル:
  - `codex-rs/tui/src/app.rs`
