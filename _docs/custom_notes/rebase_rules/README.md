...
### `almost` 相当で全体実行だけ失敗するテスト

- rebase 後の `almost` 相当で失敗し、同じテストを fully-qualified name で単体実行すると成功する場合は、全体実行時のタイミング・並列実行・バックグラウンド処理の影響を確認する。
- 本体の不具合ではなく全体実行時だけの揺らぎと判断した場合は、テスト名をルートの `flaky_test_list.txt` に追加する。これにより `make almost` / `make test-almost` ではスキップし、単体テストでは引き続き検証できる。
- 追加後は、MCP 経由で対象テストを単体実行し、成功することを確認する。
