...
6) rebase 実行（コンフリクト解消→記録→対象ファイルだけ add→continue）
...
- このルールは運用手順のため、エージェント側で動作確認コマンドは実行しない。
