...
- hooks は対象外で、現状のままでよい
  - 設定ファイルに書かれた内容を人間が管理する経路であり、`worker_user` へ落とす必要はない
- skills は対象外で、実装済み
  - これはプロンプト注入であって、スクリプト実行経路ではない
- plugins の説明注入も対象外で、実装済み
  - plugin は直接 invoke されず、skill / MCP / app の説明をモデルに渡すだけ
- MCP は対象外で、現状維持
  - 人間側でサーバーを立てて権限をコントロールできるため、ここでは `worker_user` を強制しない
- `apply_patch` は対象で、実装済み
  - `custom.exec.worker_user` が設定されている場合は `run_as` を引き継いで実行する
  - `EPERM` / `PermissionDenied` で direct spawn が失敗した場合の `sudo` フォールバックは `core/src/spawn.rs` の共通 helper に寄せている

### 補足

- `apply_patch` は別の read ツールではなく、内部でファイルシステム抽象を読んで patch を適用する
- そのため、`worker_user` 対象に含めるなら「書き込みだけ」ではなく「read/write を含む外部プロセス経路」として扱う必要がある
- 現在は、その外部プロセス経路が `custom.exec.worker_user` に従うように揃えている
- skills / plugins にスクリプトが紛れ込む設計が将来入る場合は、注入だけか実行かを分けて再評価する
